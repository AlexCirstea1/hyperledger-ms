...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.vaultx.com/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.vaultx.com/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.vaultx.com/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.vaultx.com/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.vaultx.com/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.vaultx.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.vaultx.com-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.vaultx.com-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.vaultx.com

    mkdir -p organizations/ordererOrganizations/vaultx.com/ca
    mkdir -p organizations/ordererOrganizations/vaultx.com/msp/cacerts
    mkdir -p organizations/ordererOrganizations/vaultx.com/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/vaultx.com/orderers
    mkdir -p organizations/ordererOrganizations/vaultx.com/tlsca

    CERT_DIR=organizations/ordererOrganizations/vaultx.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.vaultx.com-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.vaultx.com-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/vaultx.com/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/vaultx.com/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.vaultx.com-cert.pem"
